...
def teleport(grid, x, y):
    ...
    i = np.random.choice(range(len(exc)))
...
def runsim(PROB_RED = .3, PROB_BLUE = .3, THRESHOLD = .3, WIDTH = 50, HEIGHT = 50, NUM_GEN = 20, animate = False, plot = False):
	# Create a random starting configuration with about PROB_RED of the cells
	# being red, PROB_BLUE being blue, and the rest empty.
	config = np.random.choice([RED, BLUE, EMPTY], p=[PROB_RED, PROB_BLUE, 1 - PROB_RED - PROB_BLUE], size = WIDTH * HEIGHT)
...
def parameter_sweep():
	...
	for i in range(0, len(tolerance_levels)):
		...
		for j in range(0, sweep_count):
			count += runsim(THRESHOLD = round(tolerance_levels[i], format_count))

		average_final_uniformity[i] = count / sweep_count
